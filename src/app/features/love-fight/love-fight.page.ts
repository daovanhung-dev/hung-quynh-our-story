...
@Component({
  ...
  template: `
    <main class="love-fight-page" [attr.data-state]="state()" [attr.data-engine-ready]="session.engineReady()" [attr.data-q-x]="session.quynhX()" [attr.data-h-x]="session.hungX()" [attr.data-velocity-x]="session.velocityX()" [attr.data-quynh-action]="session.quynhAction()" [attr.data-hung-action]="session.hungAction()" [attr.data-combo-count]="session.comboCount()" [attr.data-dash-ready]="session.dashReady()" [attr.data-camera-zoom]="session.cameraZoom()" [attr.data-action-feedback]="session.actionFeedback()" aria-labelledby="love-fight-title">
      ...
      <ng-template #fightTemplate>
        <section class="fight-screen" aria-label="Trận đấu LOVE FIGHT">
          ...
          <div class="fight-caption" aria-live="polite" [class.feedback-good]="session.feedbackTone() === 'good'" [class.feedback-warning]="session.feedbackTone() === 'warning'"><span class="caption-default">{{ roundMessage() }}</span>@if (session.actionFeedback()) {<strong class="action-feedback">{{ session.actionFeedback() }}</strong>}</div>
          ...
          <div class="fight-toolbar"><button type="button" class="small-button" (click)="togglePause()">{{ paused() ? 'Tiếp tục' : 'Tạm dừng' }}</button><span>A/D hoặc ←/→ di chuyển · W/↑ nhảy · S/↓ cúi · J/J/K combo · L đồ nghề · Space dash · Esc dừng</span><div class="action-status"><span [class.is-hot]="comboCount() > 0">{{ comboCount() > 0 ? comboCount() + ' HIT COMBO' : 'READY' }}</span><b [class.is-ready]="dashReady()">{{ dashReady() ? 'DASH READY' : 'DASH...' }}</b></div></div>
          <div class="mobile-controls" aria-label="Điều khiển trên điện thoại"><div class="mobile-direction"><button type="button" aria-label="Di chuyển trái" (pointerdown)="pressVirtual($event, 'left')" (pointerup)="releaseVirtual($event, 'left')" (pointercancel)="releaseVirtual($event, 'left')" (pointerleave)="releaseVirtual($event, 'left')">←</button><button type="button" aria-label="Di chuyển phải" (pointerdown)="pressVirtual($event, 'right')" (pointerup)="releaseVirtual($event, 'right')" (pointercancel)="releaseVirtual($event, 'right')" (pointerleave)="releaseVirtual($event, 'right')">→</button><button type="button" aria-label="Nhảy" (pointerdown)="pressAction($event, 'jump')" (pointerup)="releasePointer($event)" (pointercancel)="releasePointer($event)" (pointerleave)="releasePointer($event)">NHẢY</button><button type="button" aria-label="Cúi" (pointerdown)="pressVirtual($event, 'crouch')" (pointerup)="releaseVirtual($event, 'crouch')" (pointercancel)="releaseVirtual($event, 'crouch')" (pointerleave)="releaseVirtual($event, 'crouch')">CÚI</button></div><div class="mobile-actions"><button type="button" aria-label="Đánh nhẹ" (pointerdown)="pressAction($event, 'light')" (pointerup)="releasePointer($event)" (pointercancel)="releasePointer($event)" (pointerleave)="releasePointer($event)">ĐÁNH</button><button type="button" aria-label="Đánh nặng" (pointerdown)="pressAction($event, 'heavy')" (pointerup)="releasePointer($event)" (pointercancel)="releasePointer($event)" (pointerleave)="releasePointer($event)">ĐÁ</button><button type="button" aria-label="Dùng vũ khí" (pointerdown)="pressAction($event, 'weapon')" (pointerup)="releasePointer($event)" (pointercancel)="releasePointer($event)" (pointerleave)="releasePointer($event)">ĐỒ NGHỀ</button><button type="button" aria-label="Dash né đòn" (pointerdown)="pressAction($event, 'dodge')" (pointerup)="releasePointer($event)" (pointercancel)="releasePointer($event)" (pointerleave)="releasePointer($event)">DASH</button></div></div>
        </section>
      </ng-template>
  ...
  styles: [`
    ...
    @keyframes feedback-pop { from { opacity:0; transform:translateY(8px) scale(.92); } to { opacity:1; transform:translateY(0) scale(1); } }
...
export class LoveFightPage implements AfterViewInit {
  ...
  protected readonly comboCount = this.session.comboCount;
  protected readonly dashReady = this.session.dashReady;
  ...
  protected pressVirtual(event: PointerEvent, action: LoveFightInput): void {
    this.capturePointer(event);
    this.setVirtual(action, true);
  }
  protected releaseVirtual(event: PointerEvent, action: LoveFightInput): void {
    this.setVirtual(action, false);
    this.releasePointer(event);
  }
  protected pressAction(event: PointerEvent, action: LoveFightInput): void {
    this.capturePointer(event);
