...
@Component({
  ...
  styles: [`
    ...
    .site-header { position:sticky; top:0; z-index:50; border-bottom:1px solid var(--border); background:rgba(248,244,238,.94); backdrop-filter:blur(14px); transition:opacity 180ms var(--ease-out),visibility 180ms var(--ease-out); }
    ...
    .site-footer { display:flex; flex-wrap:wrap; justify-content:center; align-items:center; gap:.65rem 1rem; padding:5rem 1.5rem 2.8rem; background:var(--paper); color:var(--text-muted); font-size:.72rem; letter-spacing:.03em; text-align:center; transition:opacity 180ms var(--ease-out),visibility 180ms var(--ease-out); }
    ...
    @media (max-width:680px) {
      ...
      nav a { padding:0 .2rem; font-size:.62rem; }
    }
  `]
})
